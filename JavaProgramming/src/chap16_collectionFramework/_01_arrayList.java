...
public class _01_arrayList {
	...
	public static void main(String[] args) {
		...
		ArrayList list1 = new ArrayList(); // 타입 미지정시 경고
...
//		System.out.println(list1);
		...
		//포함여부 확인
		if(list1.containsAll(list2)) { //컬렉션을 봐야하기 때문이다.
		...
		//ArrayList의 복제
		ArrayList list3 = (ArrayList)list2.clone(); // 형변환 필수
		
		System.out.println(list2 == list3); //깊복 vs 얕복 확인
		...
		//Type을 지정안했을 때의 문제점
		//형 변환시 castException이발생할 수도 있다.
		//Type을 지정해서 공통의 타입의 데이터만 다룰 수 있도록 한다.
		
		for(int i =0; i< list2.size(); i++) {
//			String str = new String(list2.get(i)); // 에러 타입 미지정
//			String str = new String(list2.get(i).toString());
			String str = (String)list2.get(i); // valueof로만 해야함. 
			strArr[i] = str;
