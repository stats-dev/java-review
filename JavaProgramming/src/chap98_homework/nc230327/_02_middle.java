...
//	    4명의 학생 데이터를 입력한 후 4명 중 최고 점을 맞은 학생의 이름과 점수을 출력하세요.
		...
		String name = "";
		int score = 0; // 초기값을 항상 넣어줘야 합니다.
		int cnt = 0;
		
		while(it.hasNext()) {
			Entry<String, Integer> ent = 
										it.next(); // Entry는 키와 값의 한 쌍을 꺼낼 수 있는 친구입니다.
			if(cnt > 0) {
...
//	    ex) {"3" : 1({1, 2}), "4" : 1({1, 3}, "5" : 2({1, 4}, {2, 3})
		...
		for(int i = 0; i < intArr.length; i++) {
			for(int j = i + 1; j < intArr.length; j++) { // i+1로 j를 해줘야 작은 값이 안나온다.
				if(j != i) {
		...
		for(int i = 0; i < intList.size(); i++) {
			...
			for(int j = 0; j < intList.size(); j++) {
				if(i != j) {
					if(intList.get(i) == intList.get(j)) { //index가 같지 않을때만 비교해주고, 하나씩 꺼내서 같으면 sum cnt를 증가시켰습니다.
			...
			if(sumCnt > 0) //0보다 크니까, 실행해서 그 값을 넣는다.
