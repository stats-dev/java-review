...
public class _02_middle {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < 5; i++) {
			map.put(i, i); //0~4까지 들어가고, 값은 반대로 출력이 된다.
		}
		printKVDesc(map);
...
//    Entry를 리턴하는 메소드 getMaxMul을 구현하세요.
	public static <K extends Number, V extends Number> Entry<K, V> getMaxMul(Map<K, V> map) {
		...
		for(Entry<K, V> ent : map.entrySet()) {
			if(index == 0) {
				num = ent.getKey().doubleValue() * ent.getValue().doubleValue(); //하나면 그 자체로 max이다. 그대로 보내기.
				maxEntry = ent;
			} else {
...
//	   구현하세요.
	public static <K, V> void printKVDesc(Map<K, V> map) {
		...
		for(int i = 0; i < entList.size(); i++) { //키를 정방향으로 꺼낸다.
			System.out.println(entList.get(i).getKey() 
					+ " : " + entList.get(entList.size() - (i + 1)).getValue()); //Value는 계산해서, 인덱스를 거꾸로 해서 바로 출력하도록 만든다. get()의 인덱스만 거꾸로한다.
		}
	}
}
